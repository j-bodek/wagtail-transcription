"""Local overrides for django-notifications template tags.

Adjusts format_html usage to satisfy Django 6+ requirement for args/kwargs.
"""
...
except ImportError:  # pragma: no cover - old Django
    from django.core.urlresolvers import reverse  # pylint: disable=no-name-in-module
...
def register_notify_callbacks(
    ...
    script = '<script type="text/javascript"{nonce}>'.format(nonce=nonce_str)
    script += definitions

...
class TestTranscriptionModel(WagtailPageTests):
    def setUp(self):
        Transcription.objects.create(
            title="Valid Transcription", video_id="aaaaaaaaaaa"
        )

    def test_create_transcription_with_invalid_video_id(self):
        transcription = Transcription(title="Valid Transcription", video_id="too_short")
        try:
            transcription.clean()
    ...
    def test_can_delete_transcription(self):
        ...
        self.assertEqual(
            transcription_num_before_delete - 1, Transcription.objects.all().count()
        )
